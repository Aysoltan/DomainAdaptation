...
def get_gutenberg_as_unl():
    instances = []
    path = 'data/Gutenberg_corpus_v1.0/*.txt'
    ...
    for name in files:
        ...
        #start = re.search(r'(CHAPTER I\.|\*\*\* START OF THIS PROJECT GUTENBERG EBOOK .* \*\*\*)', text).end()
        if re.search(r'CHAPTER I\.', text) is not None:
            start = re.search(r'(CHAPTER I\.)', text).end()
        elif re.search(r'\*\*\* START OF THIS PROJECT GUTENBERG EBOOK .* \*\*\*', text) is not None:
            start = re.search(r'(\*\*\* START OF THIS PROJECT GUTENBERG EBOOK .* \*\*\*)', text).end()
        elif re.search(r'\*\*\* START OF THE PROJECT GUTENBERG EBOOK .* \*\*\*', text) is not None:
            start = re.search(r'(\*\*\* START OF THE PROJECT GUTENBERG EBOOK .* \*\*\*)', text).end()
        #end = re.search(r'THE END', text).start()
        if re.search(r'THE END', text) is not None:
            end = re.search(r'THE END', text).start()
        else:
            end = len(text)-1
        ...
        #instances = [x.strip().replace('\n', ' ') for x in re.split('\n{2,}',raw)]
        #print("Number of paragraphs in one doc", len(instances))
    #print(len(instances), " instances from Project Gutenberg")
    return instances


def get_more_unlabelled():
    instances = []
    with open('data/train-v3.csv', 'r') as f:
        for line in f:
            instance = line.split('\t')
            label = instance[0]
            #text = instance[1]
            text = instance[1].replace('[#TRIGGERWORD#]', label)
            text = text.replace('@USERNAME', '')
            text = text.replace('removed', '')
            text = text.replace('url', '')
            text = text.replace('http', '')
            instances.append(text)
    #print(len(instances), " instances from train_v3.csv")
    return instances
...
if __name__ == '__main__':
    ...
    domain.append("tales-emotion") #1, 
    domain.append("dailydialogues") #2, 
    domain.append("tec") #3
    domain.append("crowdflower") #4, 
    
    
    for src_domain_index, item_src in enumerate(domain, start=0):
        src_name = domain[src_domain_index]
        for trg_domain_index, item_trg in enumerate(domain, start=0):
            trg_name = domain[trg_domain_index]
            #GETTING DATA
            ...
            if domain[src_domain_index] == domain[trg_domain_index]:
                ...
                #ADDITIONALLY UNLABELLED DATA
                print("Getting additionally unlabelled data:")
                #unlabeled = unlabeled
                #get unl_gutenberg
                if src_name == "tales-emotion" or trg_name == "tales-emotion":
                    unl_gutenberg = get_gutenberg_as_unl()
                    unlabeled = unlabeled + unl_gutenberg
                #get unl_train_v3
                if src_name == "dailydialogues" or trg_name == "dailydialogues" or src_name == "isear" or trg_name == "isear":
                    unl_train_v3 = get_more_unlabelled()
                    unlabeled = unlabeled + unl_train_v3
                #get unl_twitter
                if src_name == "crowdflower" or trg_name == "crowdflower" or src_name == "tec" or trg_name == "tec":
                    unl_EmoInt, unl_labels = get_raw_data("emoint")
                    unlabeled = unlabeled + unl_EmoInt
                    print(len(unl_EmoInt), "instances from EmoInt")
                    unl_SSEC, unl_labels = get_raw_data("ssec")
                    unlabeled = unlabeled + unl_SSEC
                    print(len(unl_SSEC), "instances from SSEC")
                ...
                while (c < pivot_num and i < len(MIsorted)):
                    ...
                    if name in bigram_vectorizer_trg.get_feature_names() and name in bigram_vectorizer.get_feature_names():
                        names.append(name)
